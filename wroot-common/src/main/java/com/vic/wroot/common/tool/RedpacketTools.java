...
public class RedpacketTools {
	...
	/**
	 * 每个红包都要有钱，最低不能低于1分，最大金额不能超过200*100 (微信就是不能大于200元的 此处不限制 或数值很大 如20000*100)
	 */
	private static final int MINMONEY = 1;
	private static final int MAXMONEY = 20000 * 100;
	/**
	 ...
	 * 分红包 中的第一个红包
	 */
	public static int getFirstRedpacket(int money, int count, int currentMaxMoney) {
		// 红包 合法性校验
		...
		int max = currentMaxMoney;
		max = max > MAXMONEY ? MAXMONEY : max;
		// 分配红包
		int one = randomRedPacket(money, MINMONEY, max, count);
		return one;
	}
	 ...
	 * 分红包 中的第一个红包
	 */
	public static int getFirstRedpacket(int money, int count) {
		// 红包 合法性校验
		if (!isRight(money, count)) {
			return 0;
		}
	 ...
	 * @return
	 */
	public static List<Integer> getRedpackets(int money, int count) {
		// 红包 合法性校验
		if (!isRight(money, count)) {
			return null;
		}
	...
	public static void main(String[] args) {  
        //单位是分  
    //    System.out.println(RedpacketTools.getRedpackets(100, 5));
        int  total = 2000;//20元
     	for(int i=10; i>0; i--){
     		int redPredpacketack = RedpacketTools.getFirstRedpacket(total, i, i*100);
     		total = total - redPredpacketack;
     		System.out.println(i + " "  +  redPredpacketack +  "     "  +  total);
     	}
    }
}
