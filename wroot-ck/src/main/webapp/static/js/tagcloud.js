...
;window.tagcloud = (function(win, doc) { // ns
    ...
    TagCloud.prototype = {
        ...
        /*随机一个背景色add by vic  2016年12月28日*/
        _randomBackground : function(index){
       		if(index){
				index = index % 6;
			}else {
				index = parseInt(5*Math.random());
			}
			var arr = ['#777','#337ab7', '#5cb85c','#5bc0de','#f0ad4e', '#d9534f'];
			return arr[index];
        },
        _getItems: function () {
            var self = this,
            ...
            for (var i = 0; i < length; i++) {
                ...
                item.element.style.backgroundColor=self._randomBackground(i);//加上随机的背景色 by vic 2016年12月28日
                item.element.style.borderRadius='6px';//设为圆角 by vic 2016年12月28日
                items.push(item);
            }
