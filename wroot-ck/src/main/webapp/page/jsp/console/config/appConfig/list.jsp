...
<div class="row row-nocol">
    <form class="form-inline lookup" role="form" method="post">
        <div class="form-group">
            <label class="sr-only" for="lookup-app_name">小程序名称</label>
            <input	type="text" class="form-control" id="lookup-app_name" name="app_name" value="${ pager.lookup.app_name }" placeholder="请输入名称">
        </div>
    ...
    <form class="nocol">
        <div class="action-bar">
            <a class="btn btn-sm btn-success" href="${ctx }/console/config/appConfig/update/0"> <i
        ...
        <table class="table table-striped table-bordered table-hover ">
            ...
            <c:forEach items="${pager.datas}" var="cur" varStatus="status">
                <tr>
                    ...
                    <td>${ cur.app_name }</td>
                    <td><span>${ cur.app_appid }</span></td>
                    <td><span>${ cur.app_appsecret }</span></td>
                    <td>
                        <div class="btn-group-dis">
                            <a class="btn btn-xs btn-primary " href="${ctx }/console/config/appConfig/update/${ cur.id }"
                               data-toggle="tooltip" title="编辑">
                                <i class="glyphicon glyphicon-edit"></i>编辑
                            </a>
                            <button class="btn btn-xs btn-danger action-get" type="button"
                                    data-href="${ctx }/console/config/appConfig/remove/${ cur.id }" data-toggle="tooltip"
